...
class PPO:
    ...
    def load(self, path,i_ep):
        self.old_policy.load_state_dict(torch.load(path+ "PPO_model_"+str(i_ep),map_location=self.device))
        self.optimizer.load_state_dict(torch.load(path+ "PPO_optimizer_"+str(i_ep),map_location=self.device))
